...
private:
    ...
    std::unique_ptr<lunar_ir_type>           parse_type0(lunar_ir_module *module, parsec<char32_t> &ps, LANG_OWNERSHIP own, uint64_t ownline, uint64_t owncol);
    ...
    std::unique_ptr<lunar_ir_new>            parse_new(lunar_ir_module *module, parsec<char32_t> &ps);
    std::unique_ptr<lunar_ir_mkstream>       parse_mkstream(lunar_ir_module *module, parsec<char32_t> &ps);
    std::unique_ptr<lunar_ir_thread>         parse_thread(lunar_ir_module *module, parsec<char32_t> &ps);
    ...
    template <typename T> std::unique_ptr<T> parse_type_expridlit(lunar_ir_module *module, parsec<char32_t> &ps);
